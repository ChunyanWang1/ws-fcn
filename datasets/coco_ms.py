...
from .coco import COCO14ImageDataset
...
ANNOT_FOLDER_NAME ="coco_seg_anno"
def load_img_name_list(dataset_path, index=0):
    ...
    #img_name_list = [img_gt_name.split('\n')[index] for img_gt_name in img_gt_name_list]
...
def load_label_name_list(dataset_path):
    ...
    return load_img_name_list(dataset_path, index=0)


class COCOImageDataset(COCO14ImageDataset):
    ...
    def __getitem__(self, idx):
        fullpath=os.path.join(self.voc12_root, IMG_FOLDER_NAME, 'val2014', 'COCO_{}_{}.jpg'.format('val2014',self.img_name_list[idx]))
...
class COCOClsDataset(COCOImageDataset):
    ...
    def __init__(self, img_name_list_path, voc12_root):
        super(COCOClsDataset, self).__init__(img_name_list_path, voc12_root)
    ...
    def __getitem__(self, idx):
        name, img = super(COCOClsDataset, self).__getitem__(idx)

        #label_fullpath = self.label_list[idx]
        label_fullpath = os.path.join(ANNOT_FOLDER_NAME, self.label_list[idx] + '.png')
...
class MultiscaleLoader(COCOClsDataset):
...
class CropLoader(COCOClsDataset):
