...
from datasets.coco import COCO14ImageDataset
...
parser.add_argument("--masks", type=str, default='results/v21/',
...
def summarise_per_class(class_stats, filename):
    data = ""
    i=-1
    for cat in COCO14ImageDataset.CLASSES:
        i=i+1
        if cat == "ambiguous":
            continue
...
def evaluate_all(args):
    ...
    conf_mat = np.zeros((81, 81))
    class_stats = {}
    for class_idx in range(81):
    ...
    with open(args.filelist) as fd:
        ...
        for line in tqdm(fd.readlines()):
            ...
            filepath_gt = os.path.join(args.data,'coco_seg_anno', files+'.png')
            ...
            if with_oracle:
                ...
                basename = files+'.png' #os.path.basename(files)
                filepath = os.path.join(args.masks,'COCO_val2014_{}'.format(basename))
            ...
            image_name = files #get_image_name(files[0])
