...
class BaseNet(nn.Module):
    ...
    def parameter_groups(self, base_lr, wd):
        ...
        groups = ({"params": [], "weight_decay":  wd, "lr": w_old*base_lr}, # weight learning
                  ...
                  #{"params": [], "weight_decay":  wd, "lr": w1_new* base_lr},  # weight learning(for cfm,align)
                  #{"params": [], "weight_decay":  0, "lr": b1_new * base_lr}  # bias learning(for cfm,align)
        ...
        """for m in self.modules():
            ...
            if m in self.align1.modules():
                if isinstance(m, nn.Conv2d) or \
                        isinstance(m, nn.Linear) or \
                        isinstance(m, self.NormLayer):

                    if not m.weight is None:
                        groups[4]["params"].append(m.weight)
                    if not m.bias is None:
                        groups[5]["params"].append(m.bias)

            elif m in self.cfm36.modules():
                if isinstance(m, nn.Conv2d) or \
                        isinstance(m, nn.Linear) or \
                        isinstance(m, self.NormLayer):
